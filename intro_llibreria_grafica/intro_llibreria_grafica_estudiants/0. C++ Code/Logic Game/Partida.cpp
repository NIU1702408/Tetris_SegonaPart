...
void Partida::actualitza(double deltaTime)
{
	...
	int fila = 2;
	int columna = 3;
	GraphicManager::getInstance()->drawSprite(GRAFIC_QUADRAT_BLAUCEL, POS_X_TAULER + (columna * MIDA_QUADRAT), POS_Y_TAULER + ((fila - 1) * MIDA_QUADRAT), false);
	...
	for (int i = 0; i < 3; i++)
	{
		for (int j = 0; j < 3; j++)
		{
			if (m_forma[i][j] != NO_COLOR)
				GraphicManager::getInstance()->drawSprite(GRAFIC_QUADRAT_BLAUFOSC, POS_X_TAULER + (j * MIDA_QUADRAT) + (columna * MIDA_QUADRAT), POS_Y_TAULER + (i * MIDA_QUADRAT) + ((fila - 1) * MIDA_QUADRAT), false);
		}
	}
	...
	//------------------------------------------
	// TODO 2.1: Dibuixar per pantalla un quadrat groc a la posicio indicada per m_fila i 
	// m_columna i que es vagi movent cap a la dreta cada cop que es prem la tecla de la fletxa dreta. 

	GraphicManager::getInstance()->drawSprite(GRAFIC_QUADRAT_BLAUCEL, POS_X_TAULER + (m_columna * MIDA_QUADRAT), POS_Y_TAULER + ((m_fila - 1) * MIDA_QUADRAT), false);
	...
	for (int i = 0; i < 3; i++)
	{
		...
		{
			if (m_forma[i][j] != NO_COLOR)
				GraphicManager::getInstance()->drawSprite(GRAFIC_QUADRAT_BLAUFOSC, POS_X_TAULER + (j * MIDA_QUADRAT) + (m_columna * MIDA_QUADRAT), POS_Y_TAULER + (i * MIDA_QUADRAT) + ((m_fila - 1) * MIDA_QUADRAT), false);
		}
	}
	...
	//-----------------------------------
	// TODO 3.1: Dibuixar per pantalla un quadrat groc a la posicio indicada per m_fila i 
